...
func init() {
	Croot.pools = make(map[string]*pool)
	Croot.searchMap = make(map[*net.TCPConn]*pool)
}

func GetTCPConn(address string) (*net.TCPConn, error) {
	return Croot.getTCPConn(address)
}
...
func PutTCPConn(tcpConn *net.TCPConn) {
	Croot.putTCPConn(tcpConn)
}

func (r *root) putTCPConn(tcpConn *net.TCPConn) {
	if pool, ok := r.searchMap[tcpConn]; ok {
